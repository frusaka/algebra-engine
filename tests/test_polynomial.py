...
def test_divide_polynomial():
    assert Interpreter.eval("(5.2x^3 + 7x^2 - 31.2x - 42) / (3.5+2.6x)") == Term(
        value=Polynomial(
            [
    ...
    assert Interpreter.eval("(x - 1)/(x - 1)^2") == Term(
        value=Polynomial(
            [
    ...
    assert Interpreter.eval("(3(x - 4) - 7(x - 4))/(x^2 - 8x + 16)") == Term(
        Number(-4),
        Polynomial(
            [
                Term(value=Variable("x")),
    ...
    assert Interpreter.eval("(3(x + 6) -0.5(x + 6))/(x + 6)^2") == Term(
        Number(5),
        Polynomial(
            [
    ...
    # Coming back soon
    # assert Interpreter.eval("(-6x^2 + 2x + 20)/(2-2x)")
    # assert Interpreter.eval("(4x^2 - 17.64) / (2x - 4)")
    assert Interpreter.eval("((x-3)(x+5)+(x-3))/(x-3)(x+5)") == Term(
        value=Product(
            [
                Term(
    ...
    assert Interpreter.eval("(x^2 - 4)/(x^2 + 8x - 20)") == Term(
        value=Product(
            [
                Term(
                    value=Polynomial(
...
def test_divide_multivariate():
    assert Interpreter.eval("(3n + 3c)/(n+c)") == Term(Number(3))
    assert Interpreter.eval("(a^3 + b^3)/(a + b)") == Term(
        value=Polynomial(
            [
    ...
    assert Interpreter.eval("(-3.75c^2 + 18ab + 4.5abc - 15c)/(3+0.75c)") == Term(
        value=Polynomial(
            [
                Term(
                    Number(6),
    ...
    assert Interpreter.eval("(ab/(x + 5))*((x+5)(x-4)/cd)") == Term(
        value=Product(
            [
                Term(
                    value=Polynomial(
...
def test_multiply_polynomial():
    ...
    assert Interpreter.eval("(2x+3)(0.5x - 5)") == Term(
        value=Polynomial(
            [
    ...
    assert Interpreter.eval("(x + 1)(x + 2)(x + 3)") == Term(
        value=Polynomial(
            [
    ...
    assert Interpreter.eval("(x + 1)(y + 2)") == Term(
        value=Polynomial(
            [
                Term(
                    value=Product(
    ...
    assert Interpreter.eval("(x - 4 + 12/(x + 4))(x+4)") == Term(
        value=Polynomial(
            [
                Term(Number(1), Variable("x"), Number(2)),
                Term(Number(-4)),
    ...
    # Nested
    assert Interpreter.eval("((z + 3)(z - 3))^2") == Term(
        value=Polynomial(
            [
    ...
    # Negative Exponents
    assert Interpreter.eval("(x+1)^-1(x^2+2x+1)") == Term(
        value=Polynomial(
            [
                Term(value=Variable("x")),
                Term(Number(1)),
    ...
    assert Interpreter.eval("(x/3 - 7/3)x^-2") == Term(
        Number(1, 3),
        value=Product(
            [
                Term(
    ...
    assert Interpreter.eval("(x+2)^-1(3x-4)(xy+2y)") == expected
    assert Interpreter.eval("(3x-4)(x+2)^-1(xy+2y)") == expected
    assert Interpreter.eval("(xy+2y)(3x-4)(x+2)^-1") == expected
...
def test_multiply_rationals():
    ...
    assert Interpreter.eval(("(x^2-25)/((x-5)/(x+10))")) == Term(
        value=Polynomial(
            [
                Term(value=Variable("x"), exp=Number(2)),
    ...
    # fmt:off
    assert Interpreter.eval("((x^2 - 4)/(x^2 + 4x + 4)) * ((x^3 + 8)/(x^3 - 2x^2 - 4x + 8))")==Term(
        value=Product([
...
def test_add_rationals():
    # fmt:off
    assert Interpreter.eval("((x-y)/(x+y))^2 + ((x+y)/(x-y))^2")==Term(
        value=Polynomial([
        Term(Number(2)),
        Term(Number(16),
            Product([
...
def test_merge_polynomial():
    assert Interpreter.eval("(x^2 + 2x + 1) - (x + 1)") == Term(
        value=Polynomial(
            [
                Term(Number(1), Variable("x"), Number(2)),
    ...
    assert Interpreter.eval("(z^2 + 4z + 3) - (z + 3)") == Term(
        value=Polynomial(
            [
    ...
    assert Interpreter.eval("2x^2 + 3x - 5 + x^2 - x + 4") == Term(
        value=Polynomial(
            [
    ...
    assert Interpreter.eval("3y^2 + 4y - 6 + y^2 - y + 5") == Term(
        value=Polynomial(
            [
    ...
    assert Interpreter.eval("2(2√(x + c)) - 1.5(2√(x + c))") == Term(
        Number(1, 2),
        Polynomial(
            [
                Term(value=Variable("x")),
    ...
    assert Interpreter.eval("3/x - 7/3x^2") == Term(
        Number(1, 3),
        Product(
            [
                Term(
