...
def test_add_complex(interpreter):
    assert interpreter.eval(AST("(3+4i) + (1+2i)")) == AlgebraObject(Number(4, 6))
    assert interpreter.eval(AST("(5+0i) + (0+3i)")) == AlgebraObject(Number(5, 3))
    assert interpreter.eval(AST("(-2-3i) + (4-5i)")) == AlgebraObject(Number(2, -8))
    assert interpreter.eval(AST("(0+0i) + (0+0i)")) == AlgebraObject(Number())
    assert interpreter.eval(AST("(6+2i) + (-6+6i)")) == AlgebraObject(Number(imag=8))


def test_subtract_complex(interpreter):
    assert interpreter.eval(AST("(6+5i) - (4+3i)")) == AlgebraObject(Number(2, 2))
    assert interpreter.eval(AST("(3+7i) - (3+7i)")) == AlgebraObject(Number())
    assert interpreter.eval(AST("0 - (5+6i)")) == AlgebraObject(Number(-5, -6))
    assert interpreter.eval(AST("(-1+4i) - (2-2i)")) == AlgebraObject(Number(-3, 6))
    assert interpreter.eval(AST("(12+3i) - (12-1i)")) == AlgebraObject(Number(imag=4))


def test_multiply_complex(interpreter):
    assert interpreter.eval(AST("i*i")) == AlgebraObject(Number(-1))
    assert interpreter.eval(AST("(2+3i)(1+4i)")) == AlgebraObject(Number(-10, 11))
    assert interpreter.eval(AST("3(4-5i)")) == AlgebraObject(Number(12, -15))
    assert interpreter.eval(AST("(-2+3i)(-1-4i)")) == AlgebraObject(Number(14, 5))
    assert interpreter.eval(AST("(0+3i)(-1)")) == AlgebraObject(Number(imag=-3))


def test_complex_scalar_division(interpreter):
    assert interpreter.eval(AST("(4+6i)/2")) == AlgebraObject(Number(2, 3))
    assert interpreter.eval(AST("(8-4i)/-2")) == AlgebraObject(Number(-4, 2))
    assert interpreter.eval(AST("(0+0i)/2")) == AlgebraObject(Number())
    assert interpreter.eval(AST("(-6+9i)/3")) == AlgebraObject(Number(-2, 3))


def test_complex_division(interpreter):
    assert interpreter.eval(AST("(4+2i)/(1-i)")) == AlgebraObject(Number(1, 3))
    ...
    assert interpreter.eval(AST("(3+4i)/(1-2i)")) == AlgebraObject(Number(-1, 2))
