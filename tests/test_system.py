...
def test_solve_linear():
    ...
    assert parser.parse("[x + y = 5, 3x = 4y + 1]") == Comparison((x, y), (3, 2))
    assert parser.parse("[2x + 3y = 7, x - y = 2]") == Comparison(
        (x, y), (Const(13, 5), Const(3, 5))
    )
    assert parser.parse("[2x - 3 = 5y + 7, 2x + 2y = 14]") == Comparison(
...
def test_solve_quadratic_linear():
    assert parser.parse("[v + w = 10, vw = 21]") == Comparison(
        (v, w), SolutionSet({(3, 7), (7, 3)}), CompRel.IN
    )
    assert parser.parse("[y = x^2 - 3x - 46, y = -3x + 3]") == Comparison(
        (x, y), SolutionSet({(-7, 24), (7, -18)}), CompRel.IN
    )
    assert parser.parse("[y = x^2 - 19x + 58, y = -3x - 5]") == Comparison(
        (x, y),
        SolutionSet(
            {
                (9, -32),
    ...
    assert parser.parse("[(x - 2)^2 + y^2 = 58, x + y = -2]") == Comparison(
        (x, y), SolutionSet({(5, -7), (-5, 3)}), CompRel.IN
    )
    assert parser.parse("[(x + 3)^2 + y^2 = 25, 2x + y = 4]") == Comparison(
        (x, y), SolutionSet({(2, 0), (0, 4)}), CompRel.IN
    )
    assert parser.parse("[xy = z, x + y = -7, x + z = -3y - 1]") == Comparison(
        (x, y, z), SolutionSet({(-4, -3, 12), (-5, -2, 10)}), CompRel.IN
    )


def test_solve_2_quadratic():
    assert parser.parse("[x^2 + y^2 = 9, x^2 + 2y^2 = 9]") == Comparison(
        (x, y),
        SolutionSet(
            {
                (3, 0),
    ...
    assert parser.parse("[x^2 + y^2 = 25, x^2 - 9 = y^2 - 2]") == Comparison(
        (x, y), SolutionSet({(-4, -3), (-4, 3), (4, -3), (4, 3)}), CompRel.IN
    )

    assert parser.parse("[x^2 + 11 = 4y^2, (x^2 + y) = 28]") == Comparison(
        (x, y),
        SolutionSet(
            {
                (5, 3),
...
def test_solve_high_degree():
    ...
    # Base: x = -1, x = (1 ± √5)/2
    assert len(parser.parse("[x^2 + y^2 + z^2 = 4, xyz = 1, x + y + z = 0]").right) == 6
    # Base: c = unity cuberoots of 3, etc
    # To disable or not to disable groebner?
    assert getvar(
        parser.parse("[abc = 6, a^2 + b^2 = 6, a^2 + b^2 + c^3 = 9]"), "c"
    ) == nth_roots({Const(3)}, 3)

    # exact values and approximate values
    assert getvar(parser.parse("[x^4 + y^2 = 5, yx^3 = 2]"), y, 4) == {
        2,
        -2,
        -0.6374,
        0.6374,
